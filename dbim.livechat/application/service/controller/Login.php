...
class Login extends Controller
{
    ...
    public function doLogin()
    {
        if(request()->isAjax()){
            ...
            if (empty($sellerInfo['data'])) {
                return JsonRes::failed('客服所属商户不存在',-4);
            }

            if (0 == $sellerInfo['data']['seller_status']) {
                return JsonRes::failed('商户尚未激活',-5);
            }

            if (date("Y-m-d H:i:s") > $sellerInfo['data']['valid_time']) {
                return JsonRes::failed('商户使用期已过',-6);
            }
            ...
            return JsonRes::success('登录成功',url('index/index', ['u' => $keFuInfo['data']['seller_code']]));
        }
    ...
    public function clientLogin()
    {
        ...
        if (request()->isPost()) {
            ...
            if (empty($sellerInfo['data'])) {
                return JsonRes::failed('客服所属商户不存在',-1);
            }

            if (0 == $sellerInfo['data']['seller_status']) {
                return JsonRes::failed('商户尚未激活',-2);
            }

            if (date("Y-m-d H:i:s") > $sellerInfo['data']['valid_time']) {
                return JsonRes::failed('商户使用期已过',-3);
            }
            ...
            if(0 != $keFuInfo['code'] || empty($keFuInfo['data'])){
                return JsonRes::failed('客服不存在',-4);
            }

            if (0 == $keFuInfo['data']['kefu_status']) {
                return JsonRes::failed('该客服已经被禁用',-5);
            }

            if(md5($password . config('service.salt')) != $keFuInfo['data']['kefu_password']){
                return JsonRes::failed('用户名密码错误',-6);
            }
            ...
            return JsonRes::success('登录成功',$response);
        }
    }
