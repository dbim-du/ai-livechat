...
class Index extends Controller
{
    ...
    public function chatBoxJs()
    {
        ...
        if (empty($appcode)||$appcode=='appcode') {
    ...
    public function chat()
    {
        ...
        $appcode = input('param.appcode'); // dify 应用的 sites 表 code
        // var_dump('appcode:'.$appcode);

        //  根据 app_code 查询 dify 应用、工作空间、用户信息 
        if (empty($appcode)||$appcode=='appcode') {
            return $this->fetch('error');
        }
        // $selDifyAppTenantUser_Data = (new DifyData())->selDifyAppTenantUser($appcode);
        // var_dump('<p />$result_sites:'.json_encode($selDifyAppTenantUser_Data));
        ...
        // var_dump('<p />$assign:'.json_encode($assign));
        $this->assign($assign);
    ...
    public function leaveMsg()
    {
        if (request()->isPost()) {
            ...
            try{
                ...
                return JsonRes::failed('留言失败',-2);
            }
            return JsonRes::success('留言成功');
        }
    ...
    public function praise()
    {
        if (request()->isAjax()) {
            ...
            try {
                ...
                return JsonRes::failed('暂时无法评价',-1);
            }
            return JsonRes::success('本次服务已结束，感谢您的评价');
        }
    }
