...
class DistributionRobot extends BaseModel
{
    /**
     * 用户上线后触发分配客服
     * @param $param
    ...
    private function findFreeKeFu($info,$appcode)
    {
        ...
        // 查询该商户下，所有 Robot 客服信息
        $kefu = new KeFu($this->db);
        $kefuInfo = $kefu->getRobotKeFuList($info['data']['seller_id']);
        if(0 != $kefuInfo['code']) {
            return ['code' => -6, 'data' => '', 'msg' => '查询 Robot 客服失败'];
        }

        if(empty($kefuInfo['data'])) {
            return ['code' => 201, 'data' => '', 'msg' => '暂无 Robot 客服'];
        }
        ...
        try {
            ...
            return ['code' => 201, 'data' => '', 'msg' => '暂无 Robot 客服3'.$e];
        }
