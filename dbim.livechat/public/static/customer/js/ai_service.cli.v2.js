...
function callBackOnMessage (data) {
    ...
    switch(data['cmd']) {
        ...
        // 处理转接
        case 'reLink':
            // console.log("// 处理转接:" + JSON.stringify(data));
            relinkHandle(data);
            break;
...
// 处理用户接受 Rebot 客服服务
function userRebotInitHandle(data) {
    console.log("1、处理用户接受 Rebot 客服服务:" + JSON.stringify(data));
    ...
    if (400 == data.code) {
        ...
        console.log("2、处理用户接受 Rebot 客服服务:" + JSON.stringify(data));
...
function relinkHandle(data) {
    ...
    isPraise = 1;
    // socket.close();
    console.log("1、关闭当前客服服务");
    

    console.log("2、开启新客服服务:" + JSON.stringify(data));
    
    kefuCode = data.data.kefu_code;
    kefuName = data.data.kefu_name;

    console.log("3、开启新客服服务 kefuCode:" + kefuCode + " kefuName:" + kefuName);
    customer.type = 2;// 设置 转接客服类型：2、机器人客服，1、人工客服
    ...
    //     // socket.open();
    //     tryReIn();
    //     console.log("4、// 处理转接：" + JSON.stringify(customer));
...
$(function () {
    ...
    window.addEventListener('pageshow',  function(event) {
        if (event.persisted)  {  // 如果是从缓存（后退）返回 
    ...
    // 转留言服务
    $(".staff-service_zly").click(function () {
        
        // clearInterval(window.msgSpaceInterval);
        isPraise = 1;
        socket.close();
        console.log("关闭上一个客服服务");
 ...
 * 转 人工客服 / 机器人客服
 */
function kefu_zhuanjie(type){
    customer.type = type; // type:1 人工服务 / type:2 机器人客服
    
    // 关闭 上一个 Robot机器人 Socket 服务
    $(".staff-service").hide();

    clearInterval(window.msgSpaceInterval);
    isPraise = 1;
    socket.close();
    console.log("关闭当前客服服务");
...
function preInputCheck(flag) {
    ...
    if (canLink == 1) {
        ...
        console.log("咨询前输入咨询内容 flag:" + flag);
        if (flag == 1) {
            // tryToConnect();
            tryToConnectRobot();
        } else if (flag == 2) {
...
function pingPong(data) {
    ...
    console.log("心跳 pong 回应:" + JSON.stringify(data));
    
    // 没有人工客服在线时 就 隐藏 转人工按钮，显示 转 留言 按钮
    if (data['data'] == false) {
        $('.staff-service').hide();
    }else {
        // 如果当前客服为机器人客服才显示转人工
