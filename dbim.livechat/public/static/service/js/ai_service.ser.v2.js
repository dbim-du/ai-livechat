...
socket.onmessage = function (res) {
    // console.log("222222222222socketUrl:");
    // console.log("11111111111111res:"+JSON.stringify(res));
...
function callBackOnMessage(res) {
    ...
    console.log('收到各种消息的回调12:'+JSON.stringify(data));
    switch(data['cmd']) {
        ...
        // 访客离线
        case 'offline':
            console.log('// 访客离线:'+ JSON.stringify(data));
        ...
        case 'changeGroupCB':
            ...
            console.log("// 接待访客回调: " + JSON.stringify(data) + ", activeUser: " + activeUser);
            if (data.data.code == 0) {
                layer.msg('转接成功');
...
function addUser(customer, newInFlag) {
    ...
    console.log("#visitor-list:"+listTpl);
...
function addHistoryUser(customer) {
    ...
    var chatTpl = [
        '<div class="chat-box" id="hct-' + customer.customer_id + '" style="' + display + '"></div>'
    ].join('');
